...
//    float3 _NormalMap_var = UnpackNormalScale(tex2D(_NormalMap, TRANSFORM_TEX(Set_UV0, _NormalMap)), _BumpScale);
 ...
 //   float3 i_normalDir = surfaceData.normalWS;
    ...
    float4 _MainTex_var = SAMPLE_TEXTURE2D_LOD(_BaseMap, sampler_BaseMap, TRANSFORM_TEX(Set_UV0, _BaseMap), 0.0f);
    /* end of todo.*/
    ...
    float4 _1st_ShadeMap_var = lerp(SAMPLE_TEXTURE2D_LOD(_1st_ShadeMap, sampler_BaseMap, TRANSFORM_TEX(Set_UV0, _1st_ShadeMap),0.0f), _MainTex_var, _Use_BaseAs1st);
...
#endif //#ifdef UTS_LAYER_VISIBILITY    //v.2.0.5
    float4 _2nd_ShadeMap_var = lerp(SAMPLE_TEXTURE2D_LOD(_2nd_ShadeMap, sampler_BaseMap, TRANSFORM_TEX(Set_UV0, _2nd_ShadeMap), 0.0), _1st_ShadeMap_var, _Use_1stAs2nd);
